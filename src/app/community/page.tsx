...
const CommunityContent = () => {
  ...
  return (
    <div className="w-full">
          ...
          <div className="flex items-center gap-3 px-4 lg:px-0 py-3 font-medium text-sm sm:text-base min-w-max sm:min-w-0">
            {tabs.map((tab, index) => (
              ...
              >
                ...
                {/* 데스크톱 드롭다운 - 탭 버튼 옆에 표시 */}
                {index === tabs.length - 1 && showDropdown && (
                  <div className="hidden sm:block absolute top-full right-0 mt-2 w-[200px] bg-white border border-gray-200 rounded-xl shadow-xl z-[101] overflow-hidden">
                    {linkItems.map((item) => (
                      <a
                        key={item.id}
                        href={item.url}
                        ...
                        className="block px-4 py-3 hover:bg-gray-50 text-[15px] text-gray-700 transition-colors border-b border-gray-50 last:border-0"
                      >
                        {item.title}
                      </a>
                    ))}
      ...
      {/* 모바일 드롭다운 - 네비게이션 바 밖으로 분리 */}
      {showDropdown && (
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-end sm:hidden animate-in fade-in duration-200">
          <div className="absolute inset-0" onClick={() => setShowDropdown(false)} />
          <div className="w-full bg-white rounded-t-2xl p-4 space-y-1 max-h-[80vh] overflow-y-auto relative z-[101]">
            <div className="flex items-center justify-between px-2 pb-4 mb-2 border-b border-gray-100">
            ...
            {linkItems.map((item) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block px-4 py-3.5 hover:bg-gray-50 text-[16px] text-gray-800 transition-colors rounded-xl font-medium"
